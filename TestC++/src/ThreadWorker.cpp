...
ThreadWorker::ThreadWorker(const int gpu_id, std::condition_variable &cond_var,
                           SafeQueue<std::function<void()>> &queue,
                           std::exception_ptr &exception,
                           std::mutex &exceptionMutex)
    : m_gpu_id(gpu_id), m_running(true), m_external_cond_var(cond_var),
      m_external_queue(queue), m_exception(exception),
  ...
  while (m_running) {
    {
      if (m_external_queue.empty()) {
        ...
        m_external_cond_var.wait(
            lock, [&] { return !m_external_queue.empty() || !m_running; });
      }
    }
