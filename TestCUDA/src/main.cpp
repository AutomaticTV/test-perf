...
int main() {
    ...
    if (error != CUDA_SUCCESS) std::cout << "Error in cudaMalloc" << std::endl;
    ...
    if (error != CUDA_SUCCESS) std::cout << "Error in cudaMemcpyAsync HostToDevice" << std::endl;

    convertBGRtoRGBA(d_rgba, d_bgr, WIDTH, HEIGHT, stream);
    error = cudaGetLastError();
    if (error != CUDA_SUCCESS) std::cout << "Error in kernel execution" << std::endl;

    cudaMemcpyAsync(h_rgba, d_rgba, sizeof(uchar4)*WIDTH*HEIGHT, cudaMemcpyDeviceToHost, stream);
    if (error != CUDA_SUCCESS) std::cout << "Error in cudaMemcpyAsync DeviceToHost" << std::endl;

    error = cudaStreamSynchronize(stream);
    if (error != CUDA_SUCCESS) std::cout << "Error in cudaStreamSynchronize" << std::endl;
